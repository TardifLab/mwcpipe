    ...
    for filter_name in $filter_types; do
        ...
        if [[ "$filter_name" == "COMMIT2" || "$filter_name" == "COMMIT" ]]; then
            ...
            if [[ ${diffusivity} == "TRUE" ]] && [[ ! -f "${proc_dwi}/${idBIDS}_space-dwi_para_diff.txt" ]]; then 
                ...
                matlab -nodisplay -r "addpath(genpath('${MICAPIPE}/tardiflab/scripts/01_processing/Diffusivity_calc')); diff_para = double(get_diff('${tmp}/${idBIDS}_space-dwi_CC_AD.nii.gz','${tmp}/${idBIDS}_space-dwi_CC_FA.nii.gz',[0.7,0.95])); save('${proc_dwi}/${idBIDS}_space-dwi_para_diff.txt', 'diff_para', '-ASCII'); exit" 
                matlab -nodisplay -r "addpath(genpath('${MICAPIPE}/tardiflab/scripts/01_processing/Diffusivity_calc')); diff_perp = double(get_diff('${tmp}/${idBIDS}_space-dwi_CC_RD.nii.gz','${tmp}/${idBIDS}_space-dwi_CC_FA.nii.gz',[0.7,0.95])); save('${proc_dwi}/${idBIDS}_space-dwi_perp_diff.txt', 'diff_perp', '-ASCII'); exit" 
