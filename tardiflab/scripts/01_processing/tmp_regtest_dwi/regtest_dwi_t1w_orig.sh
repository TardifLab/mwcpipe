#!/bin/bash
#
# Registers T1w & dwi images using both ANTs & FSLs flirt
#
# # INPUTS:
#       $1 : id  = num in (01..30)
# 	$2 : SES = num in 1, 2
...
# Directory to store outputs
  ...
  testdir=${datadir}/tmp_micapipe/02_proc-dwi/sub-${id}/ses-${SES}/tmpregtest
...
# Output files
  ...
  T1nativepro_in_dwi_brain_NL="${testdir_dwi}/${idBIDS}_space-dwi_desc-t1w_nativepro-brain_SyN.nii.gz"
...
#-------------------------------------------------------------------------------------------------------------#
	...
	if [[ ! -f ${dwi_SyN_warp} ]]  ; then
      ...
      # Apply transformation T1nativepro_brain to DWI space
        antsApplyTransforms -d 3 -r "$fod" -i "$T1nativepro_brain" -t ${dwi_SyN_warp} -t ${dwi_SyN_affine} -t [${mat_dwi_affine},1] -o "$T1nativepro_in_dwi_brain_NL" -v -u int
