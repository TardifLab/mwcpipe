...
function alpha = get_alpha(tmp,list)
    ...
    for a = 1:height(subject)
        ...
        FA = niftiread(strcat(tmp,'/MVF_calc/',sub,'_space-dwi_FA_MNI152_1mm_splenium.nii.gz'));
        IC = niftiread(strcat(tmp,'/MVF_calc/',sub,'_space-dwi_ICVF_MNI152_1mm_splenium.nii.gz'));
        MT = niftiread(strcat(tmp,'/MVF_calc/',sub,'_space-dwi_MTsat_MNI152_1mm_splenium.nii.gz'));
