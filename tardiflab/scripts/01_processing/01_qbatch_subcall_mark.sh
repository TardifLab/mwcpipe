...
# Session-2
#  for SUB in 26 27; do
#  for SUB in 20 21 22 24 25 26 27; do
#	NSES="2"
#        ID="sub-${SUB}"
#	SES="ses-${NSES}"
...
##       qbatch -verbose -l h_vmem=${VM}G -N "s${SUB}_${NSES}_f${Fn}" ${script} $SUB $FUNC_ID ${NSES}                              		# Standard call
#        qbatch -q all.q -verbose -l h_vmem=${VM}G -N "s${SUB}_${NSES}_f${Fn}" /usr/bin/time --verbose ${script} $SUB $FUNC_ID "${NSES}"     	# option to gauge resource allocation
#  done



# Session-1
#  for SUB in {04..06} {09..11} {13..15} {17..21} {23..28}; do
#  for SUB in 17 19 28 30 ; do
  for SUB in 01 ; do
	...
	cd ${sub_dir} 												# cd or logs will output in cwd
	# Call to desired function
##	qbatch -q all.q -verbose -l h_vmem=${VM}G -N "s${SUB}_${NSES}_f${Fn}" "${script}" "$SUB" "$FUNC_ID" "${NSES}" 				# Standard call
	qbatch -q all.q -verbose -l h_vmem=${VM}G -N "s${SUB}_${NSES}_f${Fn}" /usr/bin/time --verbose "${script}" "$SUB" "$FUNC_ID" "${NSES}"	# option to auge resource allocation
  done
