...
if os.path.isdir(funcDir+"/surfaces/"):
    print('')
    print('-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-')
...
x_lh = glob.glob(funcDir+'/surfaces/'+'*_hemi-L_surf-fsLR-32k.func.gii')
x_rh = glob.glob(funcDir+'/surfaces/'+'*_hemi-R_surf-fsLR-32k.func.gii')
...
save_gii(data_corr, funcDir+'/surfaces/'+subject+'_surf-fsLR-32k_desc-timeseries_clean.shape.gii')
...
if noFC!="TRUE":
    for parcellation in parcellationList:
        ...
        save_gii(ts_r, funcDir+'/surfaces/'+subject+'_atlas-'+parcellation+'_desc-FC.shape.gii')
        del ts_r
        del ts
        del thisparc
else:
...
# fsLR-5k FC
# ------------------------------------------
x_lh = glob.glob(funcDir+'/surfaces/'+'*_hemi-L_surf-fsLR-5k.func.gii')
x_rh = glob.glob(funcDir+'/surfaces/'+'*_hemi-R_surf-fsLR-5k.func.gii')
lh_data = funcgii_load(nib.load(x_lh[0]))
rh_data = funcgii_load(nib.load(x_rh[0]))
data = []
data = np.append(lh_data, rh_data, axis=1)
...
ts_r = np.corrcoef(np.transpose(ts))
ts_r = np.triu(ts_r)
save_gii(ts_r, funcDir+'/surfaces/'+subject+'_surf-fsLR-5k_desc-FC.shape.gii')
# Clean up
del data
del ts_r
del ts
